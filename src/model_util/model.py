        ...
        for attr in metrics:
            for metric in metrics[attr]:
                if not testing:
                    ...
                    print(
                        f"{attr} {metric}: {round(metrics[attr][metric], 4)}")
...
def training_loop(torch_ds, validation_ds, optimizer, device, model, classifier_params, scheduler, epochs=20):
    ...
    for i in range(epochs):
        for data in tqdm(iter(torch_ds)):
            ...
            optimizer.step()
            step_counter += 1
        validation_loop(validation_ds, False, device,
                        model, classifier_params, i)
        scheduler.step()
        model.train()
...
def identity_matcher(attribute_predictions, data_frame):
    ...
    for attr in list(attribute_predictions.keys()):
        if attr == 'age' or attr == 'down_colours' or attr == 'up_colours':
            if attr == 'age':
                dict_new[attr] = [
                    int(torch.argmax(attribute_predictions[attr]).cpu().item()) + 1]
            else:
                dict_new[attr] = [
                    int(torch.argmax(attribute_predictions[attr]).cpu().item())]
        else:
            dict_new[attr] = [
                int(torch.round(attribute_predictions[attr]).cpu().item())]

    down_colours = ['downblack', 'downblue', 'downbrown', 'downgray',
                    'downgreen', 'downpink', 'downpurple', 'downwhite', 'downyellow']
    up_colours = ['upblack', 'upblue', 'upgray', 'upgreen',
